...
def scan_day(
    ...
    asof_time = datetime.combine(day.date(), _parse_time(ross_config.trade_start))
    candidates: list[Candidate] = []
    ...
    for symbol, minute_df in symbol_bundle.items():
        ...
        if minute_day.index.tz is not None:
            asof = asof_time.replace(tzinfo=tz).astimezone(minute_day.index.tz)
        else:
            asof = asof_time
        minute_day = minute_day.loc[minute_day.index <= asof]
        if minute_day.empty:
            continue
        ...
        had_news = has_recent_news(symbol, asof if asof.tzinfo else asof.replace(tzinfo=tz), ross_config.news_lookback_hours, news_df)
